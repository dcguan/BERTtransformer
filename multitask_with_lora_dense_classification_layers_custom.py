...
from lora.layers import Linear as LoRALinear  # Import the LoRA-enhanced Linear layer
from lora.utils import mark_only_lora_as_trainable, lora_state_dict  # Import the LoRA-specific utility functions
import time 
...
TQDM_DISABLE=False
writer = SummaryWriter('runs/lora_multitask_dnn_2')
...
class TaskClassifier(nn.Module):
    def __init__(self, input_dim, hidden_dims, output_dim, dropout_rate=0.1):
        ...
        for hidden_dim in hidden_dims:
            layers.append(LoRALinear(input_dim, hidden_dim))
            layers.append(nn.ReLU())
            layers.append(nn.Dropout(dropout_rate))
            input_dim = hidden_dim  # Set input dim for the next layer
        
        # The final layer outputs the prediction
        layers.append(LoRALinear(input_dim, output_dim))
...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Task-specific layers using multiple LoRA-enhanced Linear layers
        self.sentiment_classifier  = TaskClassifier(BERT_HIDDEN_SIZE, [128, 128], N_SENTIMENT_CLASSES)
        self.paraphrase_classifier = TaskClassifier(2 * BERT_HIDDEN_SIZE, [128, 128], 1)
        self.similarity_classifier = TaskClassifier(2 * BERT_HIDDEN_SIZE, [128, 128], 1)
...
def train_multitask(args):
    ...
    print(f"Marked only LoRA parameters as trainable")
    ...
    for epoch in range(args.epochs):
        ...
        # and interleave batches from each dataset
        ...
        for i, batch in enumerate(progress_bar, 1):
            ...
            progress_bar.set_description(f"Epoch {epoch+1}/{args.epochs} Loss: {batch_loss.item():.4f}")
...
if __name__ == "__main__":
    ...
    args.filepath = f'lora-dnn-{args.option}-{args.epochs}-{args.lr}-multitask-time{current_unix_time}.pt' # Save path.
