...
TQDM_DISABLE=False
writer = SummaryWriter('runs/lora_multitask_2')
...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Task-specific layers using LoRA-enhanced Linear layers
        self.sentiment_classifier  = LoRALinear(BERT_HIDDEN_SIZE, N_SENTIMENT_CLASSES, r=config.lora_r, lora_alpha=config.lora_alpha, lora_dropout=config.lora_dropout)
        self.paraphrase_classifier = LoRALinear(2 * BERT_HIDDEN_SIZE, 1, r=config.lora_r, lora_alpha=config.lora_alpha, lora_dropout=config.lora_dropout)
        self.similarity_classifier = LoRALinear(2 * BERT_HIDDEN_SIZE, 1, r=config.lora_r, lora_alpha=config.lora_alpha, lora_dropout=config.lora_dropout)
...
if __name__ == "__main__":
    ...
    args.filepath = f'lora-{args.option}-{args.epochs}-{args.lr}-multitask-time{current_unix_time}.pt' # Save path.
