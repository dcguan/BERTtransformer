...
TQDM_DISABLE=False
writer = SummaryWriter('runs/bert_sentiment_classifier')
...
class BertSentimentClassifier(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        # The classifier takes the BERT hidden state and applies a linear layer on it
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # Get output from the BERT model
        outputs = self.bert(input_ids=input_ids, attention_mask=attention_mask)
        
        # Obtain the [CLS] token's embeddings from the last layer
        pooled_output = outputs['pooler_output']
        
        # Apply dropout
        pooled_output = self.dropout(pooled_output)
        
        # Get logits from the classifier (one for each sentiment category)
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        train_loss = 0
        num_batches = 0
        for i, batch in enumerate(tqdm(train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE), 0):
